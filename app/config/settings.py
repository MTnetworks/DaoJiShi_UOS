...
class Settings:
    ...
    def _get_config_path(self):
        ...
        if platform.system() == "Windows":
            # Check if running in portable mode (config next to executable)
            local_config = "config.json"
            if os.path.exists(local_config):
                return local_config
            # Otherwise use AppData
            app_data = os.getenv('APPDATA')
            config_dir = os.path.join(app_data, "DaoJiShi")
        else:
            # Linux/Mac: ~/.config/DaoJiShi/config.json
            home = os.path.expanduser("~")
            config_dir = os.path.join(home, ".config", "DaoJiShi")

...
from PyQt5.QtCore import Qt, QPoint, QTimer, pyqtSlot, QRect
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.shortcuts.set_window(self)
        ...
        self.raise_timer.timeout.connect(self._raise_window)
        self.raise_timer.setInterval(800)
    ...
    def set_topmost(self, enable):
        ...
        if self.isVisible():
            self.show()
        self._update_raise_timer()

    def _raise_window(self):
        """Periodically re-raise window to stay above full-screen apps (e.g. PPT on UOS)."""
        if self.isVisible():
            self.raise_()

    def _update_raise_timer(self):
        """Start or stop the periodic raise timer based on visibility and topmost state."""
        is_topmost = bool(self.windowFlags() & Qt.WindowStaysOnTopHint)
        if is_topmost and self.isVisible():
            if not self.raise_timer.isActive():

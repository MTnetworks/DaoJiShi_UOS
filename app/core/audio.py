...
class AudioPlayer(QObject):
    ...
    def _on_error(self):
        logging.error(f"MediaPlayer Error: {self.player.errorString()}")

    def play(self, file_path, fade_in_duration=0):
        if not file_path:
            logging.warning("Play called with empty file path")
            return
            
        # Resolve path (handle bundled files)
        resolved_path = get_resource_path(file_path)
        logging.info(f"Playing audio: {file_path} -> {resolved_path}")
        ...
        try:
            ...
            logging.error(f"Exception in play: {e}")
